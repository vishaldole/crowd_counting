...
# # Define paths
# dataset_folder = (
#     "/media/iiit/Mtech/Vishal/UCF_CC_50-20240625T072250Z-001/UCF_CC_50"
# )
...
def load_data(folder_path):
    ...
    for file_name in image_files:
        ...
        annotation_file = file_name.replace(".jpg", "_ann.mat")
...
# class FPANetWithAttention(nn.Module):
#     def __init__(self):
#         super(FPANetWithAttention, self).__init__()
#         backbone = models.resnet50(pretrained=True)
#         self.features = nn.Sequential(*list(backbone.children())[:-2])

#         # Additional layers for improved feature extraction
#         self.conv1 = nn.Conv2d(2048, 256, kernel_size=1)  # 1x1 conv layer for c5
#         self.conv2 = nn.Conv2d(1024, 256, kernel_size=1)  # 1x1 conv layer for c4
#         self.conv3 = nn.Conv2d(512, 256, kernel_size=1)  # 1x1 conv layer for c3
#         self.upsample = nn.Upsample(
#             scale_factor=2, mode="bilinear", align_corners=False
#         )  # Upsampling layer
#         self.conv_final = nn.Conv2d(
#             256, 256, kernel_size=3, padding=1
#         )  # Final 3x3 conv layer for refinement
#         self.fc = nn.Linear(
#             256 * 144 * 192, 1
...
#     def forward(self, x):
#         # Extracting feature maps from different stages of ResNet-50
#         c3 = self.features[:6](x)  # Output from the third block of ResNet-50
#         c4 = self.features[6:7](c3)  # Output from the fourth block of ResNet-50
#         c5 = self.features[7:](c4)  # Output from the fifth block of ResNet-50

#         # Applying 1x1 convolutions to reduce channel dimensions
#         p5 = self.conv1(c5)
#         p4 = self.upsample(p5) + self.conv2(c4)  # Upsample and combine with c4
#         p3 = self.upsample(p4) + self.conv3(c3)  # Upsample and combine with c3
#         p3 = self.upsample(p3)  # Final upsampling

#         # Apply attention modules
#         p3 = self.ca(p3) * p3
#         p3 = self.sa(p3) * p3

#         out = self.conv_final(p3)  # Refinement with final conv layer
#         out = out.view(out.size(0), -1)  # Flattening the feature map
#         out = self.fc(out)  # Fully connected layer for prediction
#         return out


# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# model = FPANetWithAttention().to(device)
...
results_folder = "noval_ucf50_results"
